  ...
  if (step === 3 && confirmedBooking) {
    return (
      <motion.div 
        ...
        className="bg-white rounded-[3rem] p-8 md:p-16 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border-2 border-gray-100 text-center max-w-3xl mx-auto relative overflow-hidden"
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-600 mb-10 text-lg font-medium">
            Thank you, {confirmedBooking.guestName}. Your reservation has been successfully received.
          </p>
          ...
          <div className="bg-gray-50 rounded-3xl p-8 text-left mb-10 border border-gray-200 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-y-8 gap-x-4">
              <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Room</span>
                <span className="font-bold text-lg text-black">{confirmedBooking.roomType}</span>
              </div>
              <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Total Amount</span>
                <span className="font-bold text-lg text-black">${confirmedBooking.amount}</span>
              </div>
              <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Check-in</span>
                <span className="font-bold text-lg text-black">{confirmedBooking.checkIn}</span>
              </div>
              <div>
                <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Check-out</span>
                <span className="font-bold text-lg text-black">{confirmedBooking.checkOut}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link href="/dashboard" className="flex items-center justify-center gap-2 px-8 py-4 bg-black hover:bg-gray-900 text-white rounded-xl font-bold transition-colors shadow-lg hover:shadow-xl hover:-translate-y-1">
              View in Dashboard <ChevronRight size={20} />
            </Link>
          </div>
        </div>
      </motion.div>
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <motion.div
        ...
        className="bg-white rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border-2 border-gray-100 overflow-hidden relative"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5">
          <div className="md:col-span-2 relative hidden md:block">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-10 flex flex-col justify-end text-white">
              <h3 className="font-heading font-black text-3xl mb-2 text-gold drop-shadow-md">Midland Hotel</h3>
              <p className="text-base font-medium text-gray-200">Your premium destination in Kayonza.</p>
            </div>
          </div>
              ...
              <form onSubmit={handleNext} className="space-y-8 relative z-10">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full px-5 py-4 rounded-xl border-2 border-gray-200 bg-gray-50 text-black focus:bg-white focus:ring-0 focus:border-gold outline-none transition-all font-medium appearance-none"
                  >
                    <option value="Standard Room">Standard Room</option>
                    <option value="Deluxe Room">Deluxe Room</option>
                    <option value="Executive Room">Executive Room</option>
                    <option value="Suite">Midland Suite</option>
                  </select>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-12 pr-5 py-4 rounded-xl border-2 border-gray-200 bg-gray-50 text-black focus:bg-white focus:ring-0 focus:border-gold outline-none transition-all font-medium" 
                      />
                    </div>
                  </div>
                  <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                ...
                <div className="pt-8 flex flex-col sm:flex-row gap-4">
                  <button 
                    type="button" 
                    onClick={() => setStep(1)}
                    className="w-full sm:w-1/3 py-5 rounded-xl font-bold border-2 border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-black hover:border-gray-300 transition-colors"
                  >
                    Back
                  </button>
                  <button 
                    type="submit" 
                    className="w-full sm:w-2/3 bg-gold hover:bg-gold-light text-black py-5 rounded-xl font-bold text-lg transition-all shadow-[0_10px_20px_rgba(251,191,36,0.3)] hover:shadow-[0_15px_30px_rgba(251,191,36,0.5)] group relative overflow-hidden"
                  >
